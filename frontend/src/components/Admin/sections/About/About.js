...
export default function About() {
  ...
  useEffect(() => {
    getAbout().then(data => setItems([...data].sort((a, b) => a.row - b.row || a.col - b.col))).catch(console.error);
  }, []);

  const startEdit = (item) => { setEditId(item.id); setForm(item); };
  const startAdd  = () => { setEditId('new'); setForm({ title: '', text: '', icon: '' }); };
  ...
  const EditRow = () => (
    <tr className="section-edit-row">
      <td style={{ width: 56 }}>
        {form.icon
          ? <img className="section-thumbnail" src={form.icon} alt="" style={{ objectFit: 'contain', background: '#f3f0ff' }} />
      ...
      <td>
        <textarea className="section-textarea about-textarea" value={form.text || ''} placeholder="Описание"
    ...
    <div className="admin-card">
      ...
      <div className="section-table-wrap">
        <table className="section-table">
          <thead>
            <tr><th style={{ width: 56 }}>Иконка</th><th>URL</th><th>Заголовок</th><th>Описание</th><th></th></tr>
          </thead>
          <tbody>
            {editId === 'new' && <EditRow />}
            ...
            {items.map((item, idx) => editId === item.id ? (
              ...
              <tr key={item.id}>
                <td>
                  {item.icon
                    ? <img className="section-thumbnail" src={item.icon} alt="" style={{ objectFit: 'contain', background: '#f3f0ff' }} />
                    : <span style={{ color: '#ccc', fontSize: 12 }}>нет</span>
                  }
                </td>
                <td style={{ fontSize: 11, color: '#aaa', maxWidth: 100, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{item.icon || '—'}</td>
